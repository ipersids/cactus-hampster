...
pub async fn game_ws(ws: WebSocketUpgrade) -> Response {
    // @TODO:
    // - assign role (host/controller) to websocket to control allowed messages
    // - check Origin on the upgrade request and only allow your real frontend origins
    // - (later for security) require explicit auth in the WS handshake (JWT, Sec-WebSocket-Protocol...)
        ...
        match result {
            Ok(msg) => match msg {
                Message::Text(payload) => {
                    ...
                    // 3. convert response to payload serde_json::to_string -> Result<String>
